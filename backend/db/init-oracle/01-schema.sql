-- DDL below is verbatim from upstream Spring Batch:
-- https://raw.githubusercontent.com/spring-projects/spring-batch/refs/heads/main/spring-batch-core/src/main/resources/org/springframework/batch/core/schema-oracle.sql
--
-- Prelude: gvenzl/oracle-free runs init scripts via `sqlplus / as sysdba` against CDB$ROOT,
-- one new session per file. Switch into FREEPDB1 (where the app connects) and create
-- objects in SYSTEM (the user the app authenticates as).
